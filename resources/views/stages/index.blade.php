...
@section('content')
    <div class="container-fluid">
        <div class="row justify-content-between align-items-center mb-4">
            <div class="col-auto">
                <h1><i class="fas fa-layer-group"></i> Bird Stages</h1>
            </div>
            <div class="col-auto">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="stages-table" class="table table-striped table-hover table-bordered" style="width:100%">
                        ...
                        <tbody>
                        @foreach($stages as $stage) {{-- Passed from StageController::index --}}
                        <tr>
                            <td>{{ $stage->id }}</td>
                            <td>{{ $stage->name }}</td>
                            <td>{{ Str::limit($stage->description, 40) }}</td>
                            <td>{{ $stage->min_age_days }}</td>
                            <td>{{ $stage->max_age_days }}</td>
                            <td>{{ $stage->target_weight_grams ?? '-' }}</td>
                            <td>
                                <div class="btn-group" role="group" aria-label="Stage Actions">
                                    <a href="{{ route('stages.show', $stage->id) }}" class="btn btn-sm btn-info me-1" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('stages.edit', $stage->id) }}" class="btn btn-sm btn-primary me-1" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal_{{ $stage->id }}" title="Delete">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </div>

                                {{-- Delete Confirmation Modal (Unique ID per row) --}}
                                <div class="modal fade" id="deleteModal_{{ $stage->id }}" tabindex="-1" aria-labelledby="deleteModalLabel_{{ $stage->id }}" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title text-danger" id="deleteModalLabel_{{ $stage->id }}"><i class="fas fa-exclamation-triangle"></i> Confirm Deletion</h5>
                ...
                <div class="d-flex justify-content-center mt-4">
                    {{ $stages->links() }} {{-- Keep if NOT using DataTables or need server-side links --}}
                </div>
            </div>
        </div>
    </div>
